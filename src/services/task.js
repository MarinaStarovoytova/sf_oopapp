    ...
    if(isAdmin()){
        ...
        for( const user of User.getListUserFromLocalStorage()){
            ...
            const btnDeleteTask = 
                        ...
                        '</div></div>';
            document.querySelector(".dropdown-task-backlog").innerHTML +=  '<p>' + user.login + '</p>' + btnDeleteTask;
            for(const task of Task.getListTaskForBackLog()){
                if(user.id == task.idUser){
                    countTask++;
                    isHasTask= true;
                    document.querySelector(".dropdown-task-backlog").innerHTML += '<li class="list-task-backlog" id="'+task.id+'" contenteditable="true">' + task.description+'</li> ';
                    ...
                    document.querySelector(".delete-content-id-backlog-"+user.id+"").innerHTML += '<li class="tasks-delete button-deleteTask" id="'+task.id+'">' + task.description + ' ('+user.login + ')' + '</li>';
                }
            }
            if(countTask==0){
    ...
    if(isAdmin()){
        ...
        for( const user of User.getListUserFromLocalStorage()){
            ...
            const btnDeleteTask = 
                        ...
                        '</div></div>';
            document.querySelector(".dropdown-task-ready").innerHTML +=  '<p>' + user.login + '</p>' + btnDeleteTask;
            for(const task of Task.getListTaskForReady()){
                if(user.id == task.idUser){
                    countTask++;
                    isHasTask = true;
                    document.querySelector(".dropdown-task-ready").innerHTML += '<li class="list-task-ready" id="'+task.id+'" contenteditable="true">' + task.description+'</li> ';
                    ...
                    document.querySelector(".delete-content-id-ready-"+user.id+"").innerHTML += '<li class="tasks-delete button-deleteTask" id="'+task.id+'">' + task.description + ' ('+user.login + ')' + '</li>';
                }
            }
            if(countTask==0){
    ...
    if(isAdmin()){
        ...
        for( const user of User.getListUserFromLocalStorage()){
            ...
            const btnDeleteTask = 
                        ...
                        '</div></div>';
            document.querySelector(".dropdown-task-progress").innerHTML +=  '<p>' + user.login + '</p>' + btnDeleteTask;
            for(const task of Task.getListTaskForProgress()){
                if(user.id == task.idUser){
                    countTask++;
                    isHasTask = true;
                    document.querySelector(".dropdown-task-progress").innerHTML += '<li class="list-task-progress" id="'+task.id+'" contenteditable="true">' + task.description+'</li> ';
                    ...
                    document.querySelector(".delete-content-id-progress-"+user.id+"").innerHTML += '<li class="tasks-delete button-deleteTask" id="'+task.id+'">' + task.description + ' ('+user.login + ')' + '</li>';
                }
            }
            if(countTask==0){
    ...
    if(isAdmin()){
        for( const user of User.getListUserFromLocalStorage()){
            ...
            const btnDeleteTask = 
                        ...
                        '</div></div>';
            document.querySelector(".dropdown-task-finished").innerHTML +=  '<p>' + user.login + '</p>' + btnDeleteTask;
            for(const task of Task.getListTaskForFinished()){
                if(user.id == task.idUser){
                    countTask++;
                    document.querySelector(".dropdown-task-finished").innerHTML += '<li class="list-task-finished" id="'+task.id+'" contenteditable="true">' + task.description+'</li> ';
                    document.querySelector(".delete-content-id-finished-"+user.id+"").innerHTML += '<li class="tasks-delete button-deleteTask" id="'+task.id+'">' + task.description + ' ('+user.login + ')' + '</li>';
                }
            }
            if(countTask==0){
